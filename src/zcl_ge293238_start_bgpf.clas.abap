CLASS zcl_GE293238_start_bgpf DEFINITION
    PUBLIC
    FINAL
    CREATE PUBLIC.
   ...
   CLASS zcl_GE293238_start_bgpf IMPLEMENTATION.
   METHOD constructor.
   ...
   METHOD if_bgmc_op_single_tx_uncontr~execute.
     ...
     "DATA start_sales_order_create TYPE REF TO zcl_GE293238_so_api.

     DATA update TYPE TABLE FOR UPDATE zr_GE293238_1\\ShoppingCart.
     DATA update_line TYPE STRUCTURE FOR UPDATE zr_GE293238_1\\ShoppingCart .

     DATA error_message TYPE string.

     READ ENTITIES OF zr_GE293238_1
             ENTITY ShoppingCart
             ALL FIELDS
     ...
     IF entities IS NOT INITIAL.
       LOOP AT entities INTO DATA(entity).
         ...
         "In one the next steps you will create your own implementation
         "start_sales_order_create = NEW zcl_GE293238_so_api(
         ...
         IF r_data-sales_order IS NOT INITIAL.
           ...
           update_line-SalesOrderStatus = zbp_r_GE293238_1=>sales_order_state-created.
           update_line-OverallStatus = zbp_r_GE293238_1=>order_state-released.
         ...
         ELSE.
           ...
           update_line-OverallStatus = zbp_r_GE293238_1=>order_state-new.
           update_line-SalesOrderStatus = zbp_r_GE293238_1=>sales_order_state-failed.
         ENDIF.

         APPEND update_line TO update.
       ENDLOOP.

       MODIFY ENTITIES OF zr_GE293238_1
        ENTITY ShoppingCart
   ...
   METHOD run_via_bgpf.
      TRY.
        DATA(process_monitor) = cl_bgmc_process_factory=>get_default( )->create(
                                              ...
                                              )->set_operation(  NEW zcl_GE293238_start_bgpf( i_rap_bo_key = i_rap_bo_key )
                                              )->save_for_execution( ).
   ...
   METHOD run_via_bgpf_tx_uncontrolled.
     TRY.
        DATA(process_monitor) = cl_bgmc_process_factory=>get_default( )->create(
                                              ...
                                              )->set_operation_tx_uncontrolled(  NEW zcl_GE293238_start_bgpf( i_rap_bo_key = i_rap_bo_key )
                                              )->save_for_execution( ).
